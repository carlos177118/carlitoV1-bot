                    ...
                    for (let i of limit){
                        if(i.id === id){
                            ...
                            if (limits >= limitCount) {
                                found = true;
                                piyo.reply(from, `Tu comando BOT ha llegado al límite, pruébalo mañana :)`, id)
                                return true;
                            }else{
                                limit
                                found = true;
...
case 'rule':
            ...
            break
case 'ownermenu':
	    if (!isOwnerBot) return piyo.reply(from, 'Esta función es solo para propietarios' , id)
            await piyo.sendText(from, menuId.textownermenu())
            break
case 'menuadmin':            
            if (!isGroupMsg) return piyo.reply(from, 'Lo sentimos, este comando solo se puede usar dentro del grupo!', id)
            if (!isGroupAdmins) return piyo.reply(from, 'Falló, este comando solo puede ser utilizado por administradores de grupo!', id)
...
case 'ownerbot':
            ...
            .then(() => piyo.sendText(from, 'Si desea solicitar una función, chatee con el número de propietario!'))
            break
case 'bal':
...
case 'reminder': // by Slavyan
    ...
    break
    case 'antisticker':
        if (!isGroupMsg) return piyo.reply(from, `Este comando solo se puede usar en un grupo!`, id)
        if (!isGroupAdmins) return piyo.reply(from, `Este comando solo puede ser utilizado por el grupo de administración!`, id)
        if (!isBotGroupAdmins) return piyo.reply(from, `Este comando solo se puede usar si el bot es un administrador!`, id)
        if (ar[0] === 'on') {
            ...
            var cek = antisticker.includes(chatId);
            if(cek){
                return piyo.reply(from, `*「 ANTI SPAM STICKER 」*\nEstado : Ya inactivo`, id) //if number already exists on database
            } else {
        ...
        break
case 'antiporn':
    if (!q) return piyo,reply(from, `Características premium `, id)
    piyo.reply(from, `Esta es una función premium, si desea comprarla, por favor chat\nwa.me/50769258167` , id)
    break
case 'antilink':                
    if (!isGroupMsg) return await piyo.reply(from, ind.groupOnly(), id)
    if (!isGroupAdmins) return await piyo.reply(from, ind.adminOnly(), id)
...
const mium = body.slice(11)
    ...
    piyo.reply(from, 'Espera un minuto, luego el dueño te chateará, php, hijo de idiota' ,id)
}
break
case 'bugreport':{
...
case 'randomhentai':
    ...
    break
    case 'gifhentai':
        if (isGroupMsg) return piyo.reply(from, 'Este comando solo se puede realizar en el chat personal!', id)
       await piyo.reply(from, ind.wait(), id);
...
case 'xvideos': {
            if (!isPremium) return piyo.reply(from, `Esta característica es especial premium ` ,id)
...
await piyo.reply(from, `Perdón ${pushname}, Comando *${arghh[0]}* No enumerado en el interior */menu*!`, id)
            }
}
        // Piyobot Function
